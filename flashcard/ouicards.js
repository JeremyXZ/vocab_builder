(function (exports) {
  ...
  function next() {
    ...
    if (ouicards.counter % bigInterval === 0 && ouicards.bucketC.length !== 0) {
      ...
      ouicards.counter % smallInterval === 0 &&
      ouicards.bucketB.length !== 0
    ) {
      ...
      console.log(
        'There was a serious problem with ouicards. You should never see '
      );
    }

    // Reset ouicards.counter if it's greater than flashcard count, otherwise ++ it
    ouicards.counter >= ouicards.flashcards.length
      ? (ouicards.counter = 1)
      : ouicards.counter++;
    return newQuestion;
  }
  ...
  function correct() {
    ...
    } else console.log('Hmm, you should not be here.');
    saveToLS();
  }
  ...
  function getFromLS() {
    ...
    ouicards.currentBucket = ouicards.bucketA.length
      ? ouicards.bucketA
